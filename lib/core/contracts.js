...
// ==========================================
// 4. DOMAIN-SPECIFIC SCHEMA DEFINITION
// ==========================================

/**
 * IGroupSchema
 * Contract for defining domain-specific group model schemas.
 * Allows domains to standardize their entity grouping models (ProductionArea, Ward, Field, etc.)
 */
export class IGroupSchema {
  static "class" = null;

  /**
   * Model name (e.g., ProductionArea, Ward, Shelf)
   */
  getName() {
  ...
  /**
   * Table name for database mapping (e.g., production_areas, wards)
   */
  getTableName() {
    throw new Error('Method "getTableName" must be implemented.');
  }
   ...
   */
  getUniqueFields() {
    throw new Error('Method "getUniqueFields" must be implemented.');
  }
  ...
  config() {
    return {
      ...
      schemaDefinition: this.generate()
    };
  }
}

// ==========================================
// 5. ENTITY GROUPING & HIERARCHY (MANAGERS)
// ==========================================
...
export class IGroupMapping {
  ...
  getAPI() {
    return {
      ...
      getAvailableDevices: (groupId) => this.getAvailableDevices(groupId),
  ...
  async getAllGroup() { throw new Error('Method "getAllGroup" must be implemented.'); }
  ...
  /** Discovery: Get available devices in domain. Optionally filter to a specific group. */
  async getAvailableDevices(groupId) { throw new Error('Method "getAvailableDevices" must be implemented.'); }
